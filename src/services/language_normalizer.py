...
KNOWN_LANGUAGES: Dict[str, LanguageInfo] = {
    ...
    "日本語": LanguageInfo("Japanese", "JA", "JA", "ja"),
    "ja": LanguageInfo("Japanese", "JA", "JA", "ja"),

    # Chinese
    "chinese": LanguageInfo("Chinese", "ZH-HANS", "ZH", "zh"),
    "китайська": LanguageInfo("Chinese", "ZH-HANS", "ZH", "zh"),
    "китайский": LanguageInfo("Chinese", "ZH-HANS", "ZH", "zh"),
    "zh": LanguageInfo("Chinese", "ZH-HANS", "ZH", "zh"),

    # Turkish
    "turkish": LanguageInfo("Turkish", "TR", "TR", "tr"),
    "турецька": LanguageInfo("Turkish", "TR", "TR", "tr"),
    "турецкий": LanguageInfo("Turkish", "TR", "TR", "tr"),
    "türkçe": LanguageInfo("Turkish", "TR", "TR", "tr"),
...
class LanguageNormalizer:
    def normalize(self, text: str) -> str:
        """Synchronously normalizes short text or returns clean title case."""
        clean = (text or "").strip().lower()
        if clean in KNOWN_LANGUAGES:
            return KNOWN_LANGUAGES[clean].canonical_name
        return text.strip().title() if text else "English"

    def get_info(self, text: str) -> Optional[LanguageInfo]:
        """Gets static language info if known."""
        clean = (text or "").strip().lower()
        return KNOWN_LANGUAGES.get(clean)

    async def resolve_conversational_language(
        self,
        user_input: str,
        openrouter_api_key: Optional[str] = None,
    ) -> Optional[LanguageInfo]:
        """
        Uses DeepSeek V4 Flash via OpenRouter for smart extraction of conversational requests.
        """
...
async def normalize_language(
    ...
    ai_res = await language_normalizer.resolve_conversational_language(user_input, openrouter_api_key)
    if ai_res:
        return ai_res
